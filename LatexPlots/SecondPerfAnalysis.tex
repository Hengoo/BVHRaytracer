...
\begin{figure}[!htb]
	...
	\caption{Average number of child Nodes is the measured number during ray tracing. These 3 graphs show the most likely reasons why a branching factor over 40 / 48 has bad performance. The average leaf depths resembles the minimum number of steps needed to get tot he first triangles. The waste factor is number tested aabb divided by number successful aabb tests.}
\end{figure}
...
Following plots use SSE4. On my CPU its slower than avx. The tests go from N4 to N32 and L4 to L32. Best performing configuration is N4L4. 
...
\begin{figure}[!htb]
	...
	\caption{Effect of different leaf sizes on the total ray tracer time for sponza. (SSE). }
\end{figure}
...
\begin{figure}[!htb]
	...
	\caption{Effect of different node sizes on the total ray tracer time for sponza. (SSE) }
\end{figure}
...
\begin{figure}[!htb]
	...
	\caption{Effect of different leaf sizes on the triangle time and node time. (SSE) The leaf size influences the node time by reducing the size of the bvh. }
\end{figure}
...
\begin{figure}[!htb]
	...
	\begin{minipage}[t]{0.4\textwidth}
		...
		\end{tikzpicture}
	\end{minipage}
	\caption{Effect of different Node sizes on the triangle time and node time.(SSE) Node size has basically no influence in the triangle intersections.}
\end{figure}
