...
void Node::recursiveBvh(const unsigned branchingFactor, const unsigned leafSize, const int bucketCount, const bool sortEachSplit, const bool smallLeafs)
{
	std::for_each(std::execution::par_unseq, children.begin(), children.end(),
		[&](auto& c)
		{
			c->recursiveBvh(branchingFactor, leafSize, bucketCount, sortEachSplit, smallLeafs);
		});
}
