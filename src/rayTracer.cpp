	...
	if (doPerformanceTest)
	{
		for (int i = 0; i < perfLoopCount; i++)
		{
			...
			if (gangSize == 8)
			{
				...
				for (int leafMemory = minLeafMemory; leafMemory <= 16; leafMemory += 8)
				{
					if (branchingFactor <= 8)
					{
			...
			else if (gangSize == 4)
			{
				...
				for (int leafMemory = minLeafMemory; leafMemory <= 16; leafMemory += 4)
				{
